...
def Maharashtra(data, contractor_name, contractor_address, filelocation, month, year, report, master):
    ...
    min_wages_maharashtra = read_min_wages_file(
        "MAHARASHTRA", "SEMI-SKILLED", input_filelocation)

    templates = forms_template.Templates(
        to_read=Maharashtrafilespath, to_write=filelocation, month=month, year=year, report=report, master=master)
    ...
    def Read_Holiday_file():
        ...
        if 'holidayfilename' in locals():
            ...
            def convert(variable): return datetime.datetime.strptime(
                variable, '%d%m%Y')
            holiday = holiday[1:]
            holiday["Date"] = holiday["Date"].apply(str)
            holiday["Date"] = holiday["Date"].apply(parser.parse)
            holiday = holiday.set_index(
                pd.PeriodIndex(data=holiday.Date, freq='D'))
        else:
    ...
    def Form_I():
        ...
        data_formI[["name&date_of_offence", "cause_against_fine", fix_monthly_gross_column,
                    date_of_payment_column, "Date of Fine", "remarks"]] = "NIL"
        formI_data = templates.get_data(data_formI,columns)
        data_once_per_sheet = {'A5': data_formI[company_name_column].unique()[
            0], 'A6': str(month)+" "+str(year)}
    ...
    def Form_II_Muster_Roll():
        formIIfilepath = os.path.join(
            Maharashtrafilespath, 'Form II muster roll.xlsx')
        ...
        if not data[PE_or_contract_column].unique()[0].upper() == "PE":
            data_once_per_sheet['A3'] = templates.combine_columns_of_dataframe(
                data_formII, [contractor_name_column, 'Contractor_Address']).unique()[0]
            data_once_per_sheet['A4'] = templates.combine_columns_of_dataframe(
                data_formII, ['Unit', address_column]).unique()[0]
    ...
    def Form_II_wages_reg():
        ...
        data_formII = data.copy(deep=True)
        leave_file_data = templates.get_data(data_formII, [
                                             employee_code_column, employee_name_column, "Leave Type", "Opening", "Monthly Increment", "Leave Accrued", "Used", "Encash", "Closing"])
        data_formII = data_formII.drop_duplicates(
            subset=employee_code_column, keep="last")
        ...
        data_formII[["Prev_balance", "Earned_during_month",
                     "Availed", "colsing_bal"]] = ""
        ...
        for employee_name_leave_file in data_formII[employee_name_column]:
            ...
            data_formII.loc[data_formII[employee_name_column] ==
                            employee_name_leave_file, "Prev_balance"] = prev_bal

            # monthly_inr
            mon_inr_pl = emp_details["Monthly Increment"].loc[emp_details["Leave Type"] == "PL"].replace("",0).astype(
                float)
            mon_inr_cl = emp_details["Monthly Increment"].loc[emp_details["Leave Type"] == "CL"].replace("",0).astype(
                float)
            mon_inr_sl = emp_details["Monthly Increment"].loc[emp_details["Leave Type"] == "SL"].replace("",0).astype(
                float)
            earned = mon_inr_cl.add(mon_inr_pl.add(
                mon_inr_sl, fill_value=0), fill_value=0).sum()
            data_formII.loc[data_formII[employee_name_column] ==
                            employee_name_leave_file, "Earned_during_month"] = earned
            # availed during month
            Used_pl = emp_details["Used"].loc[emp_details["Leave Type"] == "PL"].replace("",0).astype(
                float)
            Used_cl = emp_details["Used"].loc[emp_details["Leave Type"] == "CL"].replace("",0).astype(
                float)
            Used_sl = emp_details["Used"].loc[emp_details["Leave Type"] == "SL"].replace("",0).astype(
                float)
            availed = Used_cl.add(Used_pl.add(
                Used_sl, fill_value=0), fill_value=0).sum()

            data_formII.loc[data_formII[employee_name_column]
                            == employee_name_leave_file, "Availed"] = availed
            # closing
        ...
        if str(data_formII[date_of_payment_column].dtype)[0:8] == 'datetime':
            data_formII[date_of_payment_column] = data_formII[date_of_payment_column].apply(
                date_format_change)

        data_formII["HRA/Earned_basic"] = ((data_formII["HRA"].replace("",0).apply(
            float)/data_formII["Earned Basic"].replace("",0).apply(float))*100.0)
        data_formII["sal_fine_damage"] = templates.sum_columns_of_dataframe(
            data_formII, ["Fine", "Damage or Loss"])
        data_formII['S.no'] = list(range(1, len(data_formII)+1))
    ...
    def Form_VI_Overtime():
        ...
        data_formIV["Overtime"] = data_formIV["Overtime"].replace("",0).astype(float)

        data_formIV.loc[data_formIV["Overtime"]
                        == 0, date_of_payment_column] = "---"
        data_formIV[date_of_payment_column] = data_formIV[date_of_payment_column].replace(
            "", "---")
        data_formIV['Designation_Dept'] = templates.combine_columns_of_dataframe(data_formIV,["Designation","Department"])
        data_formIV["Total Earning-Overtime"] = data_formIV['Total Earning'].replace("",0).astype(
            float)-data_formIV["Overtime"].replace("",0).astype(float)
        data_formIV[["Date_overtime_worked", "Extent of over-time"]] = "NIL"
        formIV_data = templates.get_data(data_formIV, columns)

        data_once_per_sheet = {'A5': str(
            month)+" "+str(year), "A7": str(data_formIV[contractor_name_column].unique()[0])}
    ...
    def Form_VI_reg_advance():
        ...
        data_formIV[["purpose_advance", "num_installments_advance",
                     "Postponement_granted", "Date repaid", "remarks"]] = "NIL"
        formIV_data = templates.get_data(data_formIV, columns)

        data_once_per_sheet = {'A7': str(
            month)+" "+str(year), "A6": str(data_formIV[company_name_column].unique()[0])}
    ...
    def From_O():
        formOfilepath = os.path.join(
            Maharashtrafilespath, 'Form O leave book.xlsx')
        ...
        leave_file_data = data_formO[[employee_code_column, employee_name_column, "Leave Type",
                                      "Opening", "Monthly Increment", "Leave Accrued", "Used", "Encash", "Closing"]]
        data_formO = data_formO.drop_duplicates(
            subset=employee_code_column, keep="last")

        data_formO.fillna(value=0, inplace=True)
        columns = ["Employee Name & Code", "Date Joined",
                   department_column, "Registration_no"]
        data_formO["Employee Name & Code"] = data_formO[employee_name_column].astype(
            str)+"||"+data_formO[employee_code_column].astype(str)

        data_formO[["num_days", "Earned_during_month", "Availed",
                    "colsing_bal", 'Cheque No - NEFT date']] = ""
        for employee_name_leave_file in data_formO[employee_name_column]:
            ...
            data_formO.loc[data_formO[employee_name_column] ==
                           employee_name_leave_file, "num_days"] = opening_pl
        ...
        formO_data = templates.get_data(data_formO,columns)
        ...
        def PL_write(row_index, target, start, end, is_abs_num):
            ...
            def get_emp_name(var):
                return var.split("||")[0]
            temp = str(data_formO.loc[data_formO[columns[0]].apply(
                get_emp_name) == emp_name, "Date Left"].tolist()[0])
            if not (temp == "nan" or temp == "0"):
                cell_write(target, row_index, 9, temp)
            else:
                cell_write(target, row_index, 9, "---")
            cell_write(target, row_index, 10, data_formO.loc[data_formO[columns[0]].apply(
                get_emp_name) == emp_name, "Leave Encashment"].to_string(index=False))
            ...
            for sheet_idx, row in enumerate(rows, 10):
                ...
                for c_idx, value in enumerate(row, 1):
                    if c_idx == 1:
                        ...
                        # +","+str(data_formO[address_column].unique()[0])
                        target['A4'] = "Name and address of the Establishment:- "" " + \
                            str(data_formO[company_name_column].unique()[0])
                        #target['A5']="Name of Employer:- "" "+str(data_formO['Unit'].unique()[0])
                        target["H4"] = "Name of the employee:- " + \
                            str(name)+"\n"+" Receipt of leave book - "
                        target['A7'] = "Name of worker : "+str(name)
                        global emp_name
                        emp_name = str(name)
                        added[target.title] = 0
                        form_write[absent_label](
                            row_index+row_offset[target.title], target, "", "", "")
                        cell_write(
                            target, row_index+row_offset[target.title], 1, str("01"+"-"+str(month_num)+"-"+str(year)))
                        num = data_formO.loc[data_formO[employee_name_column]
                                             == emp_name, "num_days"]
                        if num.empty:
                            cell_write(target, row_index +
                                       row_offset[target.title], 2, 0)
                        else:
                            cell_write(
                                target, row_index+row_offset[target.title], 2, num.to_string(index=False))
                        ...
                        target['H8'] = "Date of entry into service :- " + \
                            str(value)
                    elif c_idx == 3:
                        target['A8'] = "Description of the Department (If Applicable) :-  "+str(
                            value)
                    elif c_idx == 4:
                        ...
                        form_write[absent_label](
                            row_index+row_offset[target.title], target, start, end, is_abs_num)
        ...
        offset = Counter(offset)+Counter(start_end_date_attendance(dataframe_to_rows(
            formO_data, index=False, header=False), "PL", offset, initial_offset))

        for sheet in formOfile.sheetnames:
            offset[sheet] += 25
            initial_offset += 25
            formOfile[sheet].merge_cells(
                "A"+str(offset[sheet]-3)+":H"+str(offset[sheet]-3))
            formOfile[sheet].merge_cells(
                "A"+str(offset[sheet]-2)+":B"+str(offset[sheet]-2))
            formOfile[sheet].merge_cells(
                "C"+str(offset[sheet]-2)+":C"+str(offset[sheet]-1))
            formOfile[sheet].merge_cells(
                "D"+str(offset[sheet]-2)+":D"+str(offset[sheet]-1))
            formOfile[sheet].merge_cells(
                "E"+str(offset[sheet]-2)+":E"+str(offset[sheet]-1))
            formOfile[sheet].merge_cells(
                "F"+str(offset[sheet]-2)+":G"+str(offset[sheet]-1))
            formOfile[sheet].merge_cells(
                "H"+str(offset[sheet]-2)+":H"+str(offset[sheet]-1))
        ...
        for r_idx, row in enumerate(rows, 10):
            for c_idx, value in enumerate(row, 1):
                ...
                index = 0
                target.cell(row=offset[code]+index, column=1,
                            value=start_date.date().strftime('%d-%m-%y'))
                target.cell(
                    row=offset[code]+index, column=1).font = Font(name='Verdana', size=8)
                target.cell(row=offset[code]+index, column=1).alignment = Alignment(
                    horizontal='center', vertical='center', wrap_text=True)
                border_sides = Side(style='thin')
                target.cell(row=offset[code]+index, column=1).border = Border(
                    outline=True, right=border_sides, bottom=border_sides)

                target.cell(row=offset[code]+index, column=2,
                            value=end_date.date().strftime('%d-%m-%y'))
                target.cell(
                    row=offset[code]+index, column=2).font = Font(name='Verdana', size=8)
                target.cell(row=offset[code]+index, column=2).alignment = Alignment(
                    horizontal='center', vertical='center', wrap_text=True)
                border_sides = Side(style='thin')
                target.cell(row=offset[code]+index, column=2).border = Border(
                    outline=True, right=border_sides, bottom=border_sides)

                target.cell(row=offset[code]+index, column=3,
                            value=len(holidays.loc[holidays.index >= str(start_date)]))
                target.cell(
                    row=offset[code]+index, column=3).font = Font(name='Verdana', size=8)
                target.cell(row=offset[code]+index, column=3).alignment = Alignment(
                    horizontal='center', vertical='center', wrap_text=True)
                border_sides = Side(style='thin')
                target.cell(row=offset[code]+index, column=3).border = Border(
                    outline=True, right=border_sides, bottom=border_sides)

                target.cell(row=offset[code]+index,
                            column=4, value=between_two_dates.sum())
                target.cell(
                    row=offset[code]+index, column=4).font = Font(name='Verdana', size=8)
                target.cell(row=offset[code]+index, column=4).alignment = Alignment(
                    horizontal='center', vertical='center', wrap_text=True)
                border_sides = Side(style='thin')
                target.cell(row=offset[code]+index, column=4).border = Border(
                    outline=True, right=border_sides, bottom=border_sides)

                target.cell(row=offset[code]+index, column=5, value=len(
                    holidays.loc[holidays.index >= str(start_date)])-between_two_dates.sum())
                target.cell(
                    row=offset[code]+index, column=5).font = Font(name='Verdana', size=8)
                target.cell(row=offset[code]+index, column=5).alignment = Alignment(
                    horizontal='center', vertical='center', wrap_text=True)
                border_sides = Side(style='thin')
                target.cell(row=offset[code]+index, column=5).border = Border(
                    outline=True, right=border_sides, bottom=border_sides)
        ...
        for sheet in formOfile.sheetnames:
            ...
            initial_offset += 7
            formOfile[sheet].merge_cells(
                "A"+str(offset[sheet]-4)+":F"+str(offset[sheet]-4))
            formOfile[sheet].merge_cells(
                "A"+str(offset[sheet]-3)+":B"+str(offset[sheet]-2))
            formOfile[sheet].merge_cells(
                "C"+str(offset[sheet]-3)+":C"+str(offset[sheet]-1))
            formOfile[sheet].merge_cells(
                "D"+str(offset[sheet]-3)+":D"+str(offset[sheet]-1))
            formOfile[sheet].merge_cells(
                "E"+str(offset[sheet]-3)+":E"+str(offset[sheet]-1))
            formOfile[sheet].merge_cells(
                "F"+str(offset[sheet]-3)+":F"+str(offset[sheet]-1))

        columns = ["Employee Name & Code", "total_leave",
                   "availed", "balance", "remarks"]
        ...
        for employee_name_leave_file in data_formO[employee_name_column]:
            ...
            if opening_cl.empty:
                data_formO.loc[data_formO[employee_name_column] ==
                               employee_name_leave_file, "total_leave"] = "0"
            else:
                opening_cl = opening_cl.to_string(index=False)
                data_formO.loc[data_formO[employee_name_column] == employee_name_leave_file,
                               "total_leave"] = opening_cl if not opening_cl == "" else "0"

            availed = emp_details["Used"].loc[emp_details["Leave Type"] == "CL"]

            if availed.empty:
                data_formO.loc[data_formO[employee_name_column]
                               == employee_name_leave_file, "availed"] = "0"
            else:
                availed = availed.to_string(index=False)
                data_formO.loc[data_formO[employee_name_column] == employee_name_leave_file,
                               "availed"] = availed if not availed == "" else "0"
                # print("------------------------------------------------------------------------------------------------------")
                # print("availed")
                # print(availed)
            ...
            if balance.empty:
                data_formO.loc[data_formO[employee_name_column]
                               == employee_name_leave_file, "balance"] = "0"
            else:
                balance = balance.to_string(index=False)
                data_formO.loc[data_formO[employee_name_column] == employee_name_leave_file,
                               "balance"] = balance if not balance == "" else "0"
                # print("balance")
                # print(balance)
        ...
        for r_idx, row in enumerate(rows, 10):
            for c_idx, value in enumerate(row, 1):
                if c_idx == 1:
                    ...
                    target.cell(row=offset[code], column=1).font = Font(
                        name='Verdana', size=8)
                    target.cell(row=offset[code], column=1).alignment = Alignment(
                        horizontal='center', vertical='center', wrap_text=True)
                    border_sides = Side(style='thin')
                    target.cell(row=offset[code], column=1).border = Border(
                        outline=True, right=border_sides_thin, bottom=border_sides_thick)

                    target.cell(row=offset[code], column=2, value=end_date)
                    target.cell(row=offset[code], column=2).font = Font(
                        name='Verdana', size=8)
                    target.cell(row=offset[code], column=2).alignment = Alignment(
                        horizontal='center', vertical='center', wrap_text=True)
                    border_sides = Side(style='thin')
                    target.cell(row=offset[code], column=2).border = Border(
                        outline=True, right=border_sides_thin, bottom=border_sides_thick)
                    target.row_dimensions[offset[code]].height = 20
                else:
                   # print("--------------------------------------------------------")
                   # print(offset[code],c_idx+1)
                    target.cell(row=offset[code],
                                column=c_idx+1, value=str(value))
                    target.cell(
                        row=offset[code], column=c_idx+1).font = Font(name='Verdana', size=8)
                    target.cell(row=offset[code], column=c_idx+1).alignment = Alignment(
                        horizontal='center', vertical='center', wrap_text=True)
                    border_sides = Side(style='thin')
                    target.cell(row=offset[code], column=c_idx+1).border = Border(
                        outline=True, right=border_sides_thin, bottom=border_sides_thick)
                    target.row_dimensions[offset[code]].height = 20
            # offset[code]+=1
