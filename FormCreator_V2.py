...
from states import (Central, Chandigarh, Contractor, Delhi, Goa, Gujarat,
...
log_filename = datetime.datetime.now().strftime(
    os.path.join(logfolder, 'logfile_%d_%m_%Y_%H_%M_%S.log'))
...
def Type1(inputfolder, month, year):
    ...
    try:
        ...
        if 'leftempfilename' in locals():
            ...
            leftemp_data.rename(
                columns={"Employee Code": "Employee Code_left"}, inplace=True)
            logging.info('left employees data loaded')
        else:
            leftemp_data = pd.DataFrame(columns=leftemp_df_columns)
            logging.info(
                'left employees data not available setting empty dataset')
        ...
        if str(employee_data['Location Code'].dtype)[0:3] != 'int':
            employee_data['Location Code'] = employee_data['Location Code'].astype(
                int)

        if str(employee_data['Employee Code_master'].dtype)[0:3] != 'obj':
            employee_data['Employee Code_master'] = employee_data['Employee Code_master'].astype(
                str)

        if str(unit_data['Location Code'].dtype)[0:3] != 'int':
            unit_data['Location Code'] = unit_data['Location Code'].astype(int)

        employee_data.drop(columns=list(
            employee_data.columns.intersection(salary_data.columns)), inplace=True)

        if str(salary_data['Emp Code_salary'].dtype)[0:3] != 'obj':
            salary_data['Emp Code_salary'] = salary_data['Emp Code_salary'].astype(
                str)

        attendance_data.drop(
            columns=['Employee Name', 'Branch', 'Designation'], inplace=True)

        if str(attendance_data['Emp Code'].dtype)[0:3] != 'obj':
            attendance_data['Emp Code'] = attendance_data['Emp Code'].astype(
                str)
        ...
        leftemp_data.drop(
            columns=['Employee Name', 'Date Joined', 'UAN Number'], inplace=True)

        if str(leftemp_data['Employee Code_left'].dtype)[0:3] != 'obj':
            leftemp_data['Employee Code_left'] = leftemp_data['Employee Code_left'].astype(
                str)
        ...
        CDE_Data['Employee Name'] = CDE_Data['Emp Name_salary'].combine_first(
            CDE_Data['Employee Name_master'])
        CDE_Data['Designation'] = CDE_Data['Designation_salary'].combine_first(
            CDE_Data['Designation_master'])
        CDE_Data['Branch'] = CDE_Data['Branch_salary'].combine_first(
            CDE_Data['Branch_master'])
        CDE_Data['Date Joined'] = CDE_Data['Date Joined_salary'].combine_first(
            CDE_Data['Date Joined_master'])
        CDE_Data['UAN Number'] = CDE_Data['UAN Number_salary'].combine_first(
            CDE_Data['UAN Number_master'])
        CDE_Data['ESIC Number'] = CDE_Data['ESIC Number_salary'].combine_first(
            CDE_Data['ESIC Number_master'])
        CDE_Data['Bank A/c Number'] = CDE_Data['Bank A/c Number_salary'].combine_first(
            CDE_Data['Bank A/c Number_master'])
        CDE_Data['Account Code'] = CDE_Data['Account Code_salary'].combine_first(
            CDE_Data['Account Code_master'])
        CDE_Data['E-Mail'] = CDE_Data['E-Mail_salary'].combine_first(
            CDE_Data['E-Mail_master'])
        CDE_Data['Remarks'] = CDE_Data['Remarks_salary'].combine_first(
            CDE_Data['Remarks_master'])
    ...
    if month.upper() in masterfilename.upper() and str(year).upper() in masterfilename.upper():
        ...
        for state in CDE_States:
            ...
            if state not in implemented_state_list:
                logging.info('State {} not implemented in our set,that is {} hence not making forms(Check if the state name matches any from the implemented set)'.format(
                    state, implemented_state_list))
                print('State {} not implemented in our set,that is {} hence not making forms'.format(
                    state, implemented_state_list))
                continue
            ...
            for UL in unit_with_location:
                inputdata = statedata[(statedata['State'] == state) & (statedata['Unit'] == UL.split(
                    ';')[0]) & (statedata['Location'] == UL.split(';')[1])].copy(deep=True)
                inputdata['Contractor_name'] = inputdata['Contractor_name'].fillna(
                    value='')
                ...
                if UL.strip()[-1] == '.':
                    ...
                    ULis = UL.strip()
                inpath = os.path.join(
                    inputfolder, Register_folder, 'States', state, ULis)
                ...
                if os.path.exists(inpath):
                    ...
                    contractor_address = inputdata['Contractor_Address'].unique()[
                        0]
                    State_Process[state](data=inputdata, contractor_name=contractor_name, contractor_address=contractor_address,
                                         filelocation=inpath, month=month, year=year, report=report, master=master)
                else:
        ...
        for UL in contractor_units:
            inputdata = contractdata.loc[(contractdata['Unit'] == UL.split(';')[0]) & (
            ...
            if UL.strip()[-1] == '.':
                ...
                ULis = UL.strip()
            inpath = os.path.join(
                inputfolder, Register_folder, 'Contractors', ULis)
            if os.path.exists(inpath):
            ...
            if not inputdata.empty:
                Contractor_Process(data=inputdata, contractor_name=contractor_name, contractor_address=contractor_address,
                                   filelocation=inpath, month=month, year=year, report=report, master=master)
            progress["value"] += 1
            percent.configure(
                text=str(progress["value"]*100//progress["maximum"])+"%")
        ...
        for UL in central_units:
            inputdata = centraldata.loc[(centraldata['Unit'] == UL.split(',')[0]) & (
            ...
            if os.path.exists(inpath):
                ...
                logging.info('directory created')
            if not inputdata.empty:
                Central_Process(data=inputdata, contractor_name=contractor_name, contractor_address=contractor_address,
                                filelocation=inpath, month=month, year=year, report=report, master=master)
            progress["value"] += 1
            percent.configure(
                text=str(progress["value"]*100//progress["maximum"])+"%")
...
def calculate_num_loop(CDE_Data):
    ...
    contractdata = CDE_Data[(CDE_Data['State_or_Central'] == 'State') & (
        CDE_Data['PE_or_contract'] == 'Contract')].copy()
    count += len(list((contractdata['Unit'] +
                       ';'+contractdata['Location']).unique()))

    centraldata = CDE_Data[CDE_Data['State_or_Central'] == 'Central'].copy()
    count += len(list((centraldata['Unit']+',' +
                       centraldata['Location']).unique()))
    return count


DataProcess = {'Type1': Type1, 'Type2': Type2,
...
def generateforms(comptype, mn, yr):
    ...
    if (companytype == "" and getfolder == "" and (month == "" or year == "")):
        report.configure(
            text="Please select month year, company folder and company type")
        ...
        logging.info("{} , {} , {} , {}".format(
            companytype, getfolder,  month,  year))
...
if Testing == True:
    report.configure(
        text="In testing Mode,will override state variable.\n Forms will be created for all implemented states")
    master.update()
if not os.path.isdir(State_forms):
    report.configure(
        text="Directory  {} not found, Need to have Form formats in the folder".format(State_forms))
    master.update()

button2 = ttk.Button(master, text="Convert forms to PDF",
                     command=convert_forms_to_pdf)
