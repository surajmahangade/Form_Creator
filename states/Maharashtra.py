...
def Maharashtra(data, contractor_name, contractor_address, filelocation, month, year, report, master):
    ...
    def Form_II_wages_reg():
        ...
        for employee_name_leave_file in data_formII[employee_name_column]:
            ...
            # monthly_inr
            mon_inr_pl = emp_details["Monthly Increment"].loc[emp_details["Leave Type"] == "PL"].astype(
                float)
            mon_inr_cl = emp_details["Monthly Increment"].loc[emp_details["Leave Type"] == "CL"].astype(
                float)
            mon_inr_sl = emp_details["Monthly Increment"].loc[emp_details["Leave Type"] == "SL"].astype(
                float)
            ...
            Used_pl = emp_details["Used"].loc[emp_details["Leave Type"] == "PL"].astype(
                float)
            Used_cl = emp_details["Used"].loc[emp_details["Leave Type"] == "CL"].astype(
                float)
            Used_sl = emp_details["Used"].loc[emp_details["Leave Type"] == "SL"].astype(
                float)
        ...
        data_formII["HRA/Earned_basic"] = ((data_formII["HRA"].apply(
            float)/data_formII["Earned Basic"].apply(float))*100.0)
    ...
    def Form_VI_Overtime():
        ...
        data_formIV["Total Earning-Overtime"] = data_formIV['Total Earning'].astype(
            float)-data_formIV["Overtime"].astype(float)
